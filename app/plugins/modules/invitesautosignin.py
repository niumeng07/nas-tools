...
class InvitesAutoSignIn(_IPluginModule):
        ...
        template = """
          <div class="table-responsive table-modal-body">
            <table class="table table-vcenter card-table table-hover table-striped">
              ...
              <tr>
                <th>签到时间</th>
                <th>签到结果 {{ ResultsCount }} {{ Results.date }} {{ Results.result }}</th>
              </tr>
              </thead>
              <tbody>
              {% if ResultsCount > 0 %}
    ...
    def init_config(self, config=None):
        ...
        if self._enabled:
            ...
            if self._cron:
                try:
                    self._scheduler.add_job(func=self.signin, run_date=datetime.now(
                        tz=pytz.timezone(Config().get_timezone())) + timedelta(seconds=3),
                        name="药丸签到")
                except Exception as err:
                    log.error(f"定时任务配置错误：{str(err)}")
    ...
    def signin(self):
        ...
        药丸签到
        """
        today = datetime.now(tz=pytz.timezone(Config().get_timezone())).strftime('%Y-%m-%d %H:%M:%S')
        # 删除7天以前的签到记录
        ...
        _result = {'date': today, 'result': '签到成功'}
